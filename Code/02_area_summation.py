...
# All thresholds from 30 to 151 by 1. Filter them later in R code.
lower_threshes = np.arange(30, 151, 1)
outfile = '../Data/area_summation.csv'

infiles = []

# Get list of files
for dirName, subdirList, fileList in os.walk(base_path, topdown=False):
...
for f in infiles:
    for lower_thresh in lower_threshes:
        area = sum_area(base_path, f, lower_thresh, resize = False)
